...
public class PrintListic {
		...
		private int olaksice=0,sifra=0;
	...
	public PrintListic(Connection _connection,String _juzer,int _pred,String _godina) {
	...
	public void obradiListic(int _vrstaplate,int _mesec,int _sifra,int _brojobrade) {
		...
		strsql = "Update tmplistic set sifra = " + sifra + " where sifra = 0";
		izvrsi(connection,strsql,1);
...
//*******************************************************
		...
		strsql = "Update tmplistic,kreditiarh set tmplistic.sifobs = kreditiarh.sifobs, " +
			...
			" and kreditiarh.mesec = " +  mesec + " AND kreditiarh.juzer='" + juzer + "'" + " AND kreditiarh.godina=" + godina +
...
//------------------------------------------------------------------------------------------------------------------
 public void listanjeRadnika(){
	 String strsql="";
	Statement stmt=null;
	strsql = "select * from ldmes where pre = " + pred + " and mesec = " + mesec + 
			" and sifra = " + sifra + " AND ldmes.juzer='" + juzer + "'" +
	...
	try {
		...
		while (rs.next())
		{
			...
			for (i=1;i<38 ;i++ )
			{
				j = String.valueOf(i);
 				...
 				strsql = "UPDATE tmplistic,ldmes set tmplistic.c = ldmes.c" + j + 
					...
					" where tmplistic.rbr = " + i + 
					" and ldmes.pre = " + pred + " and ldmes.sifra = " + sifra + //rs.getString("sifra") + 
					" and ldmes.mesec = " + mesec + " and ldmes.vrstaplate=" + vrstaplate +
					" and brojobrade=" + broj + " AND ldmes.juzer='" + juzer + "' AND ldmes.godina=" + godina;
								
				izvrsi(connection,strsql,1);
			}
			//proverava da li ima beneficirani staz
			dopben = proveriBenef(broj,rs.getInt("sifra"),mesec,pred,broj);
			...
			if (olaksice>0)
			{
				...
				//modifikuje parametar za isplatu 
				strsql = "Update tmplistic set i=i+" + 
					(pio1*(ind1/100) + zdrav1*(ind2/100) + nezap1*(ind3/100)) +
 ...
 //------------------------------------------------------------------------------------------------------------------
 public void listajKredite(){
		//azuriranje rednog broja u tabeli kredita
		int ssifra = 0;
		int red = 0;
		
		Statement stetmt=null;


		try {
			stetmt = connection.createStatement();
			strsql = "select * from kreditiarh where pre = " + pred + 
			" and mesec = " + mesec + " AND godina=" + godina +
			//" and dam = '" + dam + "' AND brojobrade=" + broj + 
			" AND brojobrade=" + broj + " AND kreditiarh.juzer='" + juzer + "'" +
			" order by sifra ,sifobs";
			
			ResultSet rs = stetmt.executeQuery( strsql );
			while (rs.next())
			{
				if (ssifra != rs.getInt("sifra"))
				{
					ssifra = rs.getInt("sifra");
					red = 0;
				}
				red = red + 1;

				UpdateKrediti(pred,ssifra,rs.getInt("sifobs"),rs.getString("partija"),red,broj);
			 }//end while
		}
		catch ( SQLException sqlex ) {
			sqlex.printStackTrace();
